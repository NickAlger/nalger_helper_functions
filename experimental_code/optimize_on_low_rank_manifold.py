...
def right_orthogonalize(x):
    x2_T = left_orthogonalize((x[1].T, x[0].T))
...
def sls_iter(
    ...
    X0, Y0 = right_orthogonalize(x0)

    X1 = Ztrue @ np.linalg.pinv(Y0 @ Omega)

    X1b, Y1b = left_orthogonalize((X1, Y0))
...
if False:
    ...
    x0 = (X0, Y0)
    x0 = sls_iter(x0, inputs, true_outputs)
